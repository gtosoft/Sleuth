...
public class SleuthActivity extends Activity {
	...
	private synchronized boolean setupSession(String deviceMACAddress) {
		...
		hs = new HybridSession(BluetoothAdapter.getDefaultAdapter(),
				deviceMACAddress, ddb, ecbOOBMessageHandler);
	...
	EventCallback ecbOOBMessageHandler = new EventCallback() {
		...
		public void onOOBDataArrived(String dataName, String dataValue) {
			...
			if (mThreadsOn != true) {
				msg("Ignoring OOB message out of scope. Threads are off. "
						+ dataName + "=" + dataValue);
				return;
			}
			...
			if (dataName
					.equals(HybridSession.OOBMessageTypes.AUTODETECT_SUMMARY)) {
				if (hs.isDetectionValid() == true) {
					msg("Hardware detection completed. String="
			...
			if (dataName
					.equals(HybridSession.OOBMessageTypes.SESSION_STATE_CHANGE)
					&& hs.getCurrentSessionType() == HybridSession.SESSION_TYPE_MONITOR) {
				int newState = 0;
